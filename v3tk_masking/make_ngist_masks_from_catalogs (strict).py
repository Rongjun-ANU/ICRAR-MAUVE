...
def query_gaia_sources(center: SkyCoord, radius: u.Quantity, cfg: Config):
    ...
    query = f"""
        ...
        AND phot_g_mean_mag < {cfg.gaia_gmag_max}
                AND ruwe < 1.4
                AND (ipd_frac_multi_peak IS NULL OR ipd_frac_multi_peak <= 2)
                AND (astrometric_excess_noise_sig IS NULL OR astrometric_excess_noise_sig <= 2)
    """
    ...
    if gal_tab is not None and len(gal_tab) > 0:
        ...
        if ra_col and dec_col:
            ...
            for i in range(len(gal_tab)):
                ...
                if gaia_sky is not None and cfg.ps1_reject_if_near_gaia_arcsec > 0:
                    try:
                        idx, sep2d, _ = sc.match_to_catalog_sky(gaia_sky)
