# 20251216 Back to nGIST

Make sure `mkdir \ngistTutorial\results` under `ngist_supplementary_public` first, so that at least the test command `ngistPipeline --config configFiles/MasterConfig.yaml --default-dir configFiles/defaultDir` can run successfully!!! We should see:

```bash
nGIST pipeline
 [ DONE    ] nGIST completed successfully.
```

Before we start, checkout the `.yaml` and `defalutDir` files for current MAUVE internal release v2 at https://www.canfar.net/storage/arc/list/projects/mauve/ and https://ws-uv.canfar.net/arc/files/projects/mauve/products/configFiles/IC3392/IC3392.defaultDir

Now my idea is to apply this setting to current `dev-branch` versioin of `nGIST` to see if it works. To begin, we still use the default testing dataset `NGC0000.fits`

After comparing with the provided `MasterConfig.yaml` and testing, below is the only parameter need to update in current new version of `nGIST`.

1. `LSF_DATA : 'lsf_MUSE-WFM'` => `LSF_DATA : 'lsf_MUSE-WFM-MILES'` *# Path of the file specifying the line-spread-function of the observational data. The specified path is relative to the configDir path in defaultDir.* 

2. `LIBRARY : 'MILES_EMLINES/'` => `LIBRARY : 'MILES/'` *# options are 'MILES/', 'miles_ssp_ch/', 'IndoUS', and 'Walcher/'*

3. `LIBRARY : 'MILES_safe/'` => `LIBRARY : 'MILES/'` *# options are 'MILES/', 'miles_ssp_ch/', or 'IndoUS/'*

For the `LSF_DATA`, I compare two files and i confirm that that they just simply change the name. I guess that is because they want to distinguish it with the other LSF: `lsf_MUSE-WFM-UDF10`. 

As for the `LIBRARY : 'MILES_EMLINES/'`  and `LIBRARY : 'MILES_safe/'` , now i understand that they are two templates that we should put in the `spectralTemplates` folder and so I have downloaded them from OzStar. Clearly, they are the subset (in terms of both $\mathrm{[M/H]}$ and $\log(\mathrm{Age})$), but still unclear why those ranges (but I guess to first order speed up the fitting process) and the differences between them and how to determine the ranges? Will continue to check tomorrow. 

Other parameters look fine for me. So far the testing dataset is working now. 

So below is the complete config file for IC3392, `MasterConfig_MAUVE_IC3392.yaml`: 

```yaml
# This is a test config file
GENERAL :
  RUN_ID : 'IC3392' # Name of the analysis run. A subdirectory of this name within the output directory will be created. This identifier further serves as a prefix to all output files.
  INPUT : 'IC3392_DATACUBE_FINAL_WCS_Pall_mad_red_v3.fits' # Input file for this analysis run. The specified path is relative to the input path given in defaultDir.
  ...
  REDSHIFT : 0.005566 # Initial guess on the redshift of the system [in z]. Spectra are shifted to rest-frame, according to this redshift.
  PARALLEL: True # Use multiprocessing [True/False]
  NCPU : 16 # Number of cores to use for multiprocessing
  LSF_DATA : 'lsf_MUSE-WFM-MILES' # Path of the file specifying the line-spread-function of the observational data. The specified path is relative to the configDir path in defaultDir.
  ...
  CREATE_HTML : False # option to create HTML pages similar to Mapviewer outputs
...
READ_DATA :
  ...
  ORIGIN : 218,218 # Origin of the coordinate system in pixel coordinates: x,y (Indexing starts at 0).
  ...
  LMAX_SNR : 7000
  EBmV : 0.0317 # Enter the E(B-V) value for the galaxy here (or use null for no Galactic dust correction)
...
SPATIAL_MASKING :
  ...
  MASK : 'IC3392_mask.fits' # File containing a spatial mask (Set False to not include a file).
...
KIN :
  ...
  #SPEC_PREMASK : 'specMask_KIN' # Optional mask if you want the first steps to use a different mask as compared to the final fit
  ...
  LMAX : 7000
  SIGMA : 66 # Initial guess of the velocity dispersion of the system [in km/s]
  ...
  DEBUG_BIN : [97] # Optional Keyword - array of bins [1,2,3] - only works when parallel = False. 
...
CONT :
  ...
  SIGMA : 66 # Initial guess of the velocity dispersion of the system [in km/s]
  MOM : 4 # Number of kinematic moments to be extracted
  ADEG : 8 # Degree of the additive Legendre polynomial. Set '0' to not include any additive polynomials
  ...
  DEBUG_BIN : [1] # Optional Keyword - array of bins [1,2,3] - only works when parallel = False. 
  SAVE_HDF5 : False # Optional Keyword to save outputs in HDF5
...
SFH :
  ...
  LMAX : 7000
  #SPEC_PREMASK : 'specMask_SFH' # Optional mask if you want the first steps to use a different mask as compared to the final fit
  ...
  REGUL_ERR : 10000. # Regularisation error for the regularised run of pPXF. Note: Regularisation = 1 / REGUL_ERR
