...
*   - input products are read from `v3tk_v7.6.8/{galaxy}` under the working directory.
*   - v3tk datacubes are read from `/arc/projects/mauve/cubes/v3tk`.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--bin-file",
        default=None,
        help="Optional explicit path to {gal}_spatial_binning_maps_further.fits",
    )
    parser.add_argument(
        "--gas-file",
        default=None,
        help=(
            ...
            "tries {gal}_gas_bin_maps_further.fits"
        ),
    )
    parser.add_argument(
        "--cont-file",
    ...
    parser.add_argument(
        "--out",
        default=None,
        help="Optional output path (default: {gal}_proxy_EW_maps_further.fits in the galaxy product folder)",
    )
...
def main() -> None:
    ...
    bin_path = (
        ...
        if args.bin_file
            ...
            "binning FITS",
            root / product_dir / f"{gal}_spatial_binning_maps_further.fits",
            root / product_dir / f"{gal}_SPATIAL_BINNING_maps_further.fits",
            root / f"{gal}_spatial_binning_maps_further.fits",
            root / f"{gal}_SPATIAL_BINNING_maps_further.fits",
            fallback_root / product_dir / f"{gal}_spatial_binning_maps_further.fits"
            if fallback_root is not None
            else None,
            fallback_root / product_dir / f"{gal}_SPATIAL_BINNING_maps_further.fits"
            if fallback_root is not None
            else None,
            fallback_root / f"{gal}_spatial_binning_maps_further.fits"
            if fallback_root is not None
            else None,
            fallback_root / f"{gal}_SPATIAL_BINNING_maps_further.fits"
            if fallback_root is not None
    ...
    out_path = (
        ...
        if args.out
        else root / product_dir / f"{gal}_proxy_EW_maps_further.fits"
    )
