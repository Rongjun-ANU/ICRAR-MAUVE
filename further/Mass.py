#!/usr/bin/env python
"""
Generate further Voronoi-binning maps for a MAUVE galaxy.
...
* Updated the default project layout for the second MAUVE project:
  ...
  - nGIST products are read from `v3tk_v7.6.8/{galaxy}` under the working directory.
* New mass products now use the `_further` suffix and are written back into
  each galaxy product folder, e.g. `v3tk_v7.6.8/{galaxy}/{galaxy}_spatial_binning_maps_further.fits`.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate further Voronoi-binning maps for a MAUVE galaxy"
    )
    p.add_argument(
...
print(f"Saved further file to {out_path.resolve()}")
