...
for src_gz in "${files[@]}"; do
	...
	# 3) run conversion
	echo "Running: python v3tk_to_R.py $dest_fits"
	set +e
	if [[ -n "$time_cmd" && "$time_supports_verbose" -eq 1 ]]; then
		"$time_cmd" -v python v3tk_to_R.py "$dest_fits"
	elif [[ -n "$time_cmd" ]]; then
		"$time_cmd" -p python v3tk_to_R.py "$dest_fits"
	else
		python v3tk_to_R.py "$dest_fits"
	fi
	py_status=$?
	set -e
