...
def _require_deps():
	try:
		...
		raise RuntimeError(
			...
			"  python v3tk_combined_R_image.py --dry-run\n\n"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
	p = argparse.ArgumentParser(
		...
		description=(
			...
			"otherwise it uses the colored observed_R.png."
		),
	)
	p.add_argument(
		"--input-dir",
	...
	p.add_argument(
		"--suffix",
		default="_combined_R.png",
		help="Output suffix (default: _combined_R.png)",
	)
	p.add_argument(
		"--workers",
		type=int,
...
def _discover_jobs(input_dir: pathlib.Path, pattern: str, suffix: str) -> list[Job]:
	...
	for p in paths:
		...
		observed_png = input_dir / f"{gid}_observed_R.png"
...
def _combine_one(job: Job, overwrite: bool) -> tuple[str, str]:
	...
	# observed_R.png is written flipped vertically to match origin='lower'; match that here.
