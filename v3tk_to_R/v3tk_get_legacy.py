...
def _parse_args(argv: list[str]) -> argparse.Namespace:
	p = argparse.ArgumentParser(
		...
		description=(
			"For each '*_v3tk_R.fits' file, compute the MUSE WCS footprint, download a Legacy Survey cutout "
		...
		with Image.open(observed_png) as im:
			...
			if (w, h) != (nx, ny):
				raise ValueError(
					...
					"Regenerate observed PNGs from the current FITS with: python v3tk_observed_R_image.py"
				)
